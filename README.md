...
All components except the [QuantME Transformation Framework](https://github.com/Anonymous-Author-Closer/QuantME-TransformationFramework) providing the graphical BPMN modeler are available via Docker.
...
```
git clone https://github.com/Anonymous-Author-Closer/QuantME-TransformationFramework
```
...
* ``QRM Data`` tab:
    * ``QRM Repository User``: Anonymous-Author-Closer
    * ``QRM Repository Name``: Closer-Use-Case
    * ``QRM Repository Path``: qrms
* ``Hybrid Runtimes`` tab:
...
Thereby, the [pre-processed data](./data/embedding.txt) is available in this repository, and thus, the following URL can be used: ``https://raw.githubusercontent.com/Anonymous-Author-Closer/Closer-Use-Case/main/data/embedding.txt``
